...
public class DataTypeValidation {
    ...
    public static void assertDouble(INDArray d) {
        if(!d.data().dataType().equals(DataBuffer.DOUBLE))
            throw new IllegalStateException("Given ndarray does not have data type double, but type " + d.data().dataType());
    }
    public static void assertFloat(INDArray d2) {
        if(!d2.data().dataType().equals(DataBuffer.FLOAT)) {
            throw new IllegalStateException("Given ndarray does not have data type float, but type " + d2.data().dataType());
        }
    }
